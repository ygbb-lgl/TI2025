...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # 居中裁剪逻辑（从640×640.py移植）
        def crop_center(frame, crop_size=480):
            h, w, _ = frame.shape
        ...
        # 如果是摄像头输入，裁剪im0s
        if webcam:
            if isinstance(im0s, list):  # 如果是多帧
                im0s = [crop_center(f) for f in im0s]
            else:  # 单帧
                im0s = crop_center(im0s)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                *xyxy, conf, cls = det[0]
                ...
                if idx_count == 5:
                    # 发送数据，格式：索引,中心x,中心y\n 像素坐标系
                    send_str = f"{idx},{cx},{cy}\n"
                    print(f"Sending: {send_str.strip()}")
                    if ser is not None:
                        try:
                        ...
                        print(f"Serial not available, would send: {send_str.strip()}")  # 打印未发送但准备发送的数据
                    idx_count = 0  # 发送后清零，避免重复
